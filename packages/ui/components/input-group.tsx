...
/** Clicking an addon (e.g. the "/" prefix) focuses the group's actual
 * input/textarea control — a mouse convenience only; the control itself is
 * always independently keyboard-reachable via Tab, so there's no keyboard
 * equivalent to add here. Stateless, so this lives at module scope instead
 * of being recreated per addon instance. */
function focusSiblingControl(e: MouseEvent<HTMLDivElement>) {
...
function InputGroupAddon({
  ...
  return (
    // biome-ignore lint/a11y/noNoninteractiveElementInteractions: mouse-only focus convenience, see focusSiblingControl
    // biome-ignore lint/a11y/noStaticElementInteractions: mouse-only focus convenience, see focusSiblingControl
    // biome-ignore lint/a11y/useKeyWithClickEvents: mouse-only focus convenience, see focusSiblingControl
    <div
      className={cn(inputGroupAddonVariants({ align }), className)}
      data-align={align}
      data-slot="input-group-addon"
      onClick={focusSiblingControl}
      {...props}
    />
  );
}
