...
function Avatar({
  ...
  return (
    <AvatarPrimitive.Root
      className={cn(
        "group/avatar relative flex size-8 shrink-0 select-none rounded-full after:absolute after:inset-0 after:rounded-sm after:border after:border-border after:mix-blend-darken data-[size=lg]:size-10 data-[size=sm]:size-6 dark:after:mix-blend-lighten",
        className
      )}
      data-size={size}
      data-slot="avatar"
...
function AvatarImage({
  ...
  return (
    <AvatarPrimitive.Image
      ...
      // "" would reach the <img> src attribute (React warns: browser may
      // re-download the page). Callers bind form values where "cleared" is the
      // empty string, so coerce to undefined — Radix then keeps the fallback
      // and never mounts the <img>.
      src={src || undefined}
      {...props}
    />
  );
