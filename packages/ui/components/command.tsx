  ...
  return (
    <div className="p-1 pb-0" data-slot="command-input-wrapper">
      <InputGroup className="h-8! bg-input/20 dark:bg-input/30">
        ...
        />
        <InputGroupAddon>
          <AppIcons.OneOff.Search className="size-3.5 shrink-0 opacity-50" />
        </InputGroupAddon>
        {onClear && hasValue ? (
          <InputGroupAddon
            align="inline-end"
            aria-label="Clear search"
            className="flex cursor-pointer items-center opacity-50 transition-opacity hover:opacity-100"
            onClick={onClear}
          >
            <AppIcons.Common.X className="size-3.5 shrink-0" />
          </InputGroupAddon>
        ) : null}
      </InputGroup>
    </div>
...
function CommandItem({
  ...
  return (
    ...
    >
      {children}
      <AppIcons.Common.Check className="ml-auto opacity-0 group-has-data-[slot=command-shortcut]/command-item:hidden group-data-[checked=true]/command-item:opacity-100" />
    </CommandPrimitive.Item>
  );
}
